...
public class RegistDemo extends JFrame {
	...
	public RegistDemo() {
		...
		b2.addActionListener((e) -> {
			...
			String pw1str = String.copyValueOf(pw1.getPassword());
			boolean ok = dao.insertUser(uname.getText().trim(), pw1str.trim());
			if (!ok) {
		...
		bl.addActionListener((e) -> {
			...
			UserDatabase dao = new UserDatabase();
			String pw1str = String.copyValueOf(pw1.getPassword());
			boolean ok = dao.checkUserPassword(uname.getText().trim(), pw1str.trim());
			//boolean ok = dao.checkUserPassword(uname.getText().trim(), pw1.getText());
			//System.out.println(String.copyValueOf(pw1.getPassword()));
			if (!ok) {
				...
				Client client;
				try {
					client = new Client(uname.getText());
					jf.setVisible(false);
